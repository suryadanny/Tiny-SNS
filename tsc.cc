...
int Client::connectTo()
{
  ...
  // flight on an active connection.
    ...
    Status coord_status = coord_stub_->GetServer(&context, id, &serverDetails );
    if(serverDetails.hostname().empty() || serverDetails.port().empty() ){
       std::cout << "Server/cluster not available" << std::endl;
       return -1;
    } 
    else{
    ...
    std::string login_info = serverhostname + ":" + serverport;        
...
void Client::Timeline(const std::string& username) {
  ...
  std::string inputMsg= "Set Stream";
  Message m = MakeMessage(username, inputMsg);
  if(!stream->Write(m)){
    std::cout<<"Timeline command couldn't be executed - couldn't set stream"<<std::endl;
    return;
  }
  ...
  std::thread reader([username, stream]() {
    ...
    while(stream->Read(&m)){
      ...
      if(m.msg().find("-") == std::string::npos && m.msg().find(":") == std::string::npos  && std::find(data.begin(),data.end(),m.msg())==data.end()){
         data.push_back(m.msg());
